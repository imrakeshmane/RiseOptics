            ...
            <View style={styles.buttons}>
                {buttons.map((button) =>
                    button === '=' || button === '/' || button === '*' || button === '-' || button === '+' ?
                        <TouchableOpacity key={button} style={[styles.button, styles.bBottom, { backgroundColor: '#00b9d6' }]} onPress={() => {
                            handleInput(button);
                            if (button === '=') {
                            ...
                            {button === '=' ? <IonicIcons name="checkmark" size={50} color={'white'} /> : <Text style={[styles.textButton, { color: 'white', fontSize: 28 }]}>{button}</Text>}
